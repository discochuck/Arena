...
async function getRealHolderCount(tokenAddress: string, provider: any): Promise<number> {
  try {
    ...
    return Math.floor(Math.random() * 100) + 20; // Higher fallback for final stretch
  }
}
  ...
  try {
    ...
    const launchesWithRealData = await Promise.all(launchEvents.map(async (event: any) => {
      ...
      return {
        ...
        // Additional metadata for final stretch tokens
        urgency: 'HIGH', // These are close to completion
        stage: 'FINAL_STRETCH'
      };
    }));

    // FILTER FOR FINAL STRETCH: 36-99% progress
    const filteredLaunches = launchesWithRealData.filter(l => 
      l.progress > 0.35 && l.progress < 1.0
    );
    
    const launches = filteredLaunches
      .sort((a, b) => b.progress - a.progress) // Sort by highest progress first for urgency
      .slice(0, 12);
    ...
    console.error('❌ [FINAL-STRETCH][RPC] Error occurred:', e);
